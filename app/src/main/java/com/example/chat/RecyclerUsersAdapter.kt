...
import com.example.chat.databinding.ListPersonItemBinding
...
class RecyclerUsersAdapter(val context: Context) :
    ...
    fun addChatRoom(position: Int) {     //채팅방 추가
        ...
        val chatRoomKey = if (currnentUser?.uid!! < opponent.uid) {
            "${currnentUser.uid}-${opponent.uid}"
        } else {
    ...
    fun goToChatRoom(chatRoom: ChatRoom, opponentUid: User, chatRoomKey: String) {       //채팅방으로 이동
